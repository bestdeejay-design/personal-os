...
importRouter.post("/", async (req, res) => {
  ...
  const topProfileIds = Array.isArray(body.profile_ids)
  ...
  function mergeProfileIds(itemProfiles: unknown): string[] {
    const item = strArr(itemProfiles);
    if (item.length === 0) return topProfileIds;
    const set = new Set([...topProfileIds, ...item]);
    return [...set];
  }
    ...
    for (const raw of lines) {
      ...
      try {
        ...
        await pool.query(
          ...
          [id, line, "", jb(topProfileIds), jb([]), null, null, null]
        );
  ...
  for (const el of elements) {
    ...
    try {
      if (looksTask) {
        const id = randomUUID();
        const itemPids = mergeProfileIds(rec.profile_ids);
        await pool.query(
        ...
        const id = randomUUID();
        const itemPids = mergeProfileIds(rec.profile_ids);
        const itemTags = strArr(rec.tags);
        await pool.query(
          `INSERT INTO notes (id, title, body_md, profile_ids, tags, linked_meeting_id, linked_project_id, linked_task_id, manual_order)
           VALUES ($1,$2,$3,$4::jsonb,$5::jsonb,$6,$7,$8,(SELECT COALESCE(MAX(manual_order),0)+1 FROM notes))`,
          [id, title, str(rec.body_md) ?? "", jb(itemPids), jb(itemTags), null, null, null]
        );
